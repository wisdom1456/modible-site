<script lang="ts">
	...
	let sortedArtworks = data.artworks;
	
	function handleSort(newSort: 'date' | 'title') {
		sortBy = newSort;
		
		if (sortBy === 'date') {
			sortedArtworks = [...data.artworks].sort((a, b) =>
...
<SEOHead
	...
	canonical="https://modible.netlify.app/art"
	type="website"
/>

<Container>
	<header class="page-header">
		<h1 class="display-xl">Gallery</h1>
	...
	{#if sortedArtworks.length > 0}
		{#key sortBy}
			<ContentGrid columns={3} animate={true}>
				{#each sortedArtworks as artwork (artwork.metadata.slug)}
					<ArcadeCard
		...
		{/key}
	{:else}
		<p class="empty-state">No artworks available yet. Check back soon!</p>
	{/if}
</Container>
